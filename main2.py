...
def createObj():
    ...
    if currentNum == 1: #Target1の生成
        target = Target(W / 2, H / 2, currentTarget.size, currentTarget.score, currentTarget.img, tag=f"TAG{countTag}")
        ...
        for n in range(currentNum):
                ...
                target = Target(
                    posX, posY, currentTarget.size, currentTarget.score, currentTarget.img, tag=f"TAG{countTag}"
                )
        ...
        for n in range(0, k, 1):
            for m in range(0, k, 1):
                posX = int(random.randint(currentTarget.size, W / k)) + (
                    (W / k) * n
                )
                if posX + currentTarget.size > W:
                    posX = W - currentTarget.size
                posY = int(random.randint(currentTarget.size, H / k)) + (
                    (H / k) * m
                )
                if posY + currentTarget.size > H:
                    posY = H - currentTarget.size
